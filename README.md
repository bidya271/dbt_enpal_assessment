...
## Scaling Thoughts
For a production environment handling millions of rows in future, the following recommendations is proposed:
- **Incremental Models**: The intermediate layer should leverage `incremental` strategies (e.g., processing only `event_ts > max(this)`) or creating keys for the combination of `deal_id` and `funnel_step` and `events_ts`to avoid processing the same deal multiple times and creating a md5 hash for the key.
...
- **Materialization Strategy**: 
  ...
  - Kept `intermediate` and `marts` as **Tables** to ensure downstream BI performance.
  - With an assumption of high-volume production scenario, I have upgraded `int_pipedrive_deal_funnel_events` to an **Incremental** model.
- **Timezone**: Explicitly standardized on UTC (`at time zone 'utc'`) early in the staging layer to avoid timezone bugs in aggregation. This is an assumption*

### 2. Key Challenges & Solutions
- **Event Deduplication**: Pipedrive data can be noisy hence,I have implemented `row_number()` window function in `int_pipedrive_deal_funnel_events` to deterministically pick the *first* entry per funnel step.
- **Mixed Sources (Stage vs Activity)**: Unified distinct streams into a common "Event" schema before aggregation. This makes adding a 3rd source (e.g., "Emails") added in the pipeline in the future without rewriting the core logic.
- **Data Gaps**: Used `generate_series` in the final mart to ensure months with ZERO deals still show up in reports, preventing misleading line charts in dashboards from the BI perspective.

### 3. Data Quality
- **Tests**: Add `unique` and `not_null` tests on primary keys.
- **Singular Test**: Implemented `monthly_spike_check` to mathematically detect anomalies (growth > 500%) to check proactive spike.
